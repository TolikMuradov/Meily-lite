...
export function buildKeySpecs() {
  ...
  for (const c of _commands.values()) {
    ...
    for (const k of keys) {
      specs.push({ key: k, preventDefault: true, run: (view) => c.run(view, {}) });
    }
  }
  return specs;
}
...
export function bootstrapTable(register, ops) {
  ...
  register({ id: 'table.nextCell', title: 'Next Cell', category: 'Table', description: 'Sonraki hücreye git', key: 'Tab', run: (view)=>moveCell(view,'next') });
  register({ id: 'table.prevCell', title: 'Previous Cell', category: 'Table', description: 'Önceki hücreye git', key: 'Shift-Tab', run: (view)=>moveCell(view,'prev') });
